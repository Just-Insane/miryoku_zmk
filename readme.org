...
Local builds are done via the CLI, from the '~/git/zmk/app' folder, with the following commands:
...
Once the files have been output, double press the power button on each half to put them in update mode, then copy '~/git/zmk/app/build/[left/right]/zephyr/zmk.uf2' into the mounted drives.

Changes are done by updating the various files in '~/git/miryoku_zmk', as described in the rest of the README. 
