...
#+begin_src text :tangle config/kyria_rev2_left.conf
# Display config
...
#+begin_src text :tangle config/kyria_rev2_right.conf
# Display config
CONFIG_ZMK_DISPLAY=y
CONFIG_ZMK_DISPLAY_BLANK_ON_IDLE=y
CONFIG_ZMK_WIDGET_BATTERY_STATUS_SHOW_PERCENTAGE=y
...
#+BEGIN_QUOTE
NOTE: If running on EvieMacStudio, the ZMK Build Environment is already setup, however, instructions can be found in ~README.md~, if needed.
#+END_QUOTE
...
1. Open ~Finder.app~ and go to src_language[:exports code]{~/git/zmk/app/build/[left/right]/zephyr}
